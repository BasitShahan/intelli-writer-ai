...
const Generator = ({ type }: Props) => {
 ...
 const handleSubmit = (e: FormEvent) => {
    ...
    if (!title) {
      ...
      switch (type) {
        case "blog-title":
          prompt = `Generate a 5 Blog titles for this title "${title}" and subheading "${subheading}" and keywords "${keywords}"
        `;
          getRespond(prompt);
          break;
        case "business-name":
          prompt = `Generate a 5 business name for this content <brand of clothing for childs for this "${title}" and the subheading this  "${subheading}" and keywords "${keywords}".`;
          getRespond(prompt);
          break;
        case "cover-letter":
          prompt = `Write a cover letter for this "${title}" with this subheading "${subheading}" and keywords "${keywords}" also write Job description:
        ...
        case "problem-agitate-solution":
          prompt = `Write a problem agitate solution about this title "${title}" with this subheading "${subheading}" and keywords "${keywords}".`;
          getRespond(prompt);
          break;
        case "rephrase-generator":
          prompt = `Rephrase some words in a simple wording of this title "${title}" with the subheading "${subheading}" and keywords "${keywords}" and also You may make the Chat Completions format more akin to the completions format by building a request with a single user message..`;
          getRespond(prompt);
          break;
        case "tags-generator":
          prompt = `Generate a 10 tags for this title "${title}" with the subheading "${subheading}" and keywords "${keywords}".`;
          getRespond(prompt);
          break;
        case "video-title":
          prompt = `Generate 5 SEO friendly video title for this title "${title}" with the subheading "${subheading}" and keywords "${keywords}".`;
          getRespond(prompt);
          break; 
          case "web-scrapper":
          ...
          prompt = `Generate questions and answers based on the following topic: "${title}" with the subheading "${subheading}" and keywords "${keywords}".`;
          getRespond(prompt);
          break;
