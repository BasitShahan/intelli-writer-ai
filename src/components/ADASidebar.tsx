...
const isButton = (element: HTMLElement): boolean => {
  ...
  return element.tagName.toLowerCase() === 'button' || !!element.closest('button');
};
...
const handleMagnify = () => {
  ...
  if (!magnifyActive) {
    ...
    // Add 'fixed-header' class to header to make it fixed
    if (header) {
    ...
    if (sidebarRef.current) {
      // Adjust sidebar properties here
    }

    // Exclude ADA icon from zooming if it exists
    if (adaIcon) {
      // Adjust ADA icon properties here
    }

    setMagnifyActive(true);
  } else {
    // Remove zoom from body content
    (document.body.style as any).zoom = "";

    // Remove 'fixed-header' class from header to reset its position
    if (header) {
    ...
    // Reset sidebar
    if (sidebarRef.current) {
      // Reset sidebar properties here
    }

    // Reset ADA icon transformation if it exists
    if (adaIcon) {
      // Reset ADA icon properties here
    }
            ...
            <div key={button.id} className="text-center relative">
              ...
              >
                <img
                  ...
                  className=" w-6 h-6 md:w-10 md:h-10 mx-auto mb-2 object-contain"
                />
