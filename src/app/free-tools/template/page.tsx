...
const FreeTool = () => {
    ...
    return (
        <div className='w-full h-full bg-gradient-to-b from-[#471c7c]/40 to-[#030616]/20 backdrop-blur-sm text-center text-white py-16 flex flex-col justify-center items-center'>
            <div className='w-full md:w-[80%] flex flex-col gap-4 md:gap-6 items-center justify-center px-4'>
                <h2 className='text-2xl md:text-4xl font-bold'>
                    Looking for some help with your content marketing?
                </h2>
                <p className='text-base md:text-2xl'>
                    Check out our collection of templates. From blog posts to social media posts, we've got you covered.
                </p>
                ...
                <div className='w-full sm:w-auto flex flex-wrap justify-center gap-4 my-5 px-4'>
                    {tags.map((item) => (
                        <button
                            key={item.id}
                            className={`rounded-3xl px-4 py-1 border ${selectedTag === item.name ? 'bg-[#6624BA] border-[#6624BA] text-white scale-110' : 'border-gray-400'} text-sm md:text-base hover:text-white hover:bg-[#6624BA] `}
            ...
            <div className='w-[90%] md:w-[80%] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 py-5'>
                {filteredTools.length > 0 ? (
                    ...
                    <p className='text-xl text-gray-400 text-center'>Whoops! No tools found.</p>
                )}
            </div>
        </div>
    );
