...
import { ResetBg, Logo } from "../../../../public";
...
const ResetPasswordPageLayout: React.FC<ResetPasswordPageProps | any> = (
  ...
  const [confseePass, setConfseePass] = useState<"text" | "password">("password");
  const [resetError, setResetError] = useState<ResetPasswordPageProps | null>(null);
  ...
  return (
    <div className="w-full h-screen flex bg-slate-900">
      <Link href="/auth/login" className="absolute z-50 left-2 top-2 opacity-75 hover:opacity-100 text-white border rounded-3xl px-4 py-1">
        Back
      </Link>
      <div className="z-10 h-full w-full md:w-1/3 flex flex-col items-center justify-center bg-transparent space-y-5 px-5 pt-5">
        <form onSubmit={submitForm}>
          <div className="flex flex-col lg:p-5 p-0 gap-y-6 text-white">
            ...
            <span className="ml-3 -mt-2 text-sm text-white/60 italic">(<span className="font-semibold">Strong Password: </span>IntelliWriter@9275)</span>
        ...
        <p className="pb-6 !m-0 text-center text-white lg:pt-0 pt-3 w-[90%]">
      ...
      <img src={ResetBg} className="w-full md:w-2/3 justify-end opacity-10 md:opacity-100 absolute h-screen right-0 object-cover" />
    </div>
  );
};
